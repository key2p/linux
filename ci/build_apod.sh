...
if [ -f "${PWD}/ci/install/init" ]; then
    export SCRIPT_DIR="$PWD/ci"
else
...
BUILD_WORK_DIR="/dev/shm/cache/build_zpod"
...
cleanup() {
    ...
    local mount_dir="${BUILD_WORK_DIR}/img_mount"
    ...
    if losetup -a | grep -q "${BUILD_WORK_DIR}/disk.img"; then
        LOOP_DEV=$(losetup -a | grep "${BUILD_WORK_DIR}/disk.img" | cut -d: -f1)
        if [ -n "$LOOP_DEV" ]; then
    ...
    # The script is designed to work within BUILD_WORK_DIR, so we don't delete it here
...
setup_environment() {
    log_info "Setting up build environment in ${BUILD_WORK_DIR}"
    rm -rf "${BUILD_WORK_DIR}" "${OUTPUT_DIR}"
    # Main structure for initrd
    mkdir -p "${BUILD_WORK_DIR}/initrd"/{bin,etc,lib,os/bin,installer,proc,sys,dev,tmp,rom}
    # Structure for the ISO
    mkdir -p "${BUILD_WORK_DIR}/iso/boot/grub"
    # Structure for the podman and ext4 disk img
    mkdir -p "${BUILD_WORK_DIR}/os"
    # Final output directory
    mkdir -p "${OUTPUT_DIR}"
}
...
download_and_extract_kernel() {
    ...
    local extract_dir="${BUILD_WORK_DIR}/kernel_extracted"
    ...
    cp "$vmlinuz_path" "${BUILD_WORK_DIR}/zpod-vmlinuz"
    cp -r "$modules_path" "${BUILD_WORK_DIR}/initrd/lib/modules/"
...
download_podman_static() {
    ...
    cp -rf "${PODMAN_CACHE}"/* "${BUILD_WORK_DIR}/initrd/" || true
    
    chmod +x "${BUILD_WORK_DIR}/initrd/usr/local/bin/"*

    rm -f "${BUILD_WORK_DIR}/initrd/usr/local/libexec/podman/quadlet"
    rm -f "${BUILD_WORK_DIR}/initrd/usr/local/bin/runc"

    mkdir -p "${BUILD_WORK_DIR}/initrd/licenses" || true
    mv "${BUILD_WORK_DIR}/initrd/usr/local/bin/LICENSE" "${BUILD_WORK_DIR}/initrd/licenses"  || true

    rm -f "${BUILD_WORK_DIR}/initrd/usr/local/bin/README.md" || true
    rm -f "${BUILD_WORK_DIR}/initrd/README.md" || true
...
prepare_initrd_rootfs() {
    ...
    tar -xzf "${ALPINE_PATH}" -C "${BUILD_WORK_DIR}/initrd"
    ...
    if [ ! -f "${BUILD_WORK_DIR}/initrd/etc/containers/containers.conf" ]; then
        echo '[engine]' > "${BUILD_WORK_DIR}/initrd/etc/containers/containers.conf"
    fi
    ...
    echo 'no_pivot_root = true' >> "${BUILD_WORK_DIR}/initrd/etc/containers/containers.conf"

    # 足够稳定的时候再切换到 youki
    #echo 'runtime = "/usr/local/bin/youki"' >> "${BUILD_WORK_DIR}/initrd/etc/containers/containers.conf"
 
    # dropbear host key
    mkdir -p "${BUILD_WORK_DIR}/initrd/etc/dropbear/" || true
    echo "AAAAE2VjZHNhLXNoYTItbmlzdHAyNTYAAAAIbmlzdHAyNTYAAABBBMOLyz3cG2NHr6BpTzDoA56cObh+tTrRwdbv9aJaZdxoHF2hU9UaPcQKsJ6eAmDbfQFrwPoYsye2ddmxo54Edr0AAAAhAIWqQdlQyVLMIRn+ED9kk4unCDmYiw8sE7n4Cs/Ujm0E" | base64 -d - > "${BUILD_WORK_DIR}/initrd/etc/dropbear/dropbear_ecdsa_host_key"
    chmod 0600 "${BUILD_WORK_DIR}/initrd/etc/dropbear/dropbear_ecdsa_host_key"

    # copy mdev files
    mkdir -p "${BUILD_WORK_DIR}/initrd/lib/mdev"
    cp -f ${SCRIPT_DIR}/install/mdev/* "${BUILD_WORK_DIR}/initrd/lib/mdev"
    mv "${BUILD_WORK_DIR}/initrd/lib/mdev/mdev.conf" "${BUILD_WORK_DIR}/initrd/etc/mdev.conf"
    chmod +x "${BUILD_WORK_DIR}/initrd/lib/mdev/"*

    cp -f ${SCRIPT_DIR}/install/init_functions "${BUILD_WORK_DIR}/initrd/bin/"
    chmod +x "${BUILD_WORK_DIR}/initrd/bin/init_functions"

    cp -f ${SCRIPT_DIR}/install/init_install "${BUILD_WORK_DIR}/initrd/sbin/init_install"
    chmod +x "${BUILD_WORK_DIR}/initrd/sbin/init_install"

    cp -f ${SCRIPT_DIR}/install/init "${BUILD_WORK_DIR}/initrd/init"
    chmod +x "${BUILD_WORK_DIR}/initrd/init"
}
...
build_disk_img() {
    ...
    if [  -e "${DISKIMG_PATH}" ]; then
        cp -f "${DISKIMG_PATH}" "${BUILD_WORK_DIR}/disk.img.xz"
        return
    fi
    ...
    log_info "disk.img.xz created in ${BUILD_WORK_DIR} ."
}
...
build_final_artifacts() {
    ...
    rm "${BUILD_WORK_DIR}/initrd/sbin/reboot" || true
    cat > "${BUILD_WORK_DIR}/initrd/sbin/reboot" << EOF
#!/bin/sh
sync; echo '[*] Zpod will reboot after 6s'; sleep 2; sync; /bin/busybox reboot; sleep 4; echo 'b' > /proc/sysrq-trigger
EOF
    chmod +x "${BUILD_WORK_DIR}/initrd/sbin/reboot"

    chmod +x "${SCRIPT_DIR}/optmize_apod.sh"
    . "${SCRIPT_DIR}/optmize_apod.sh"

    # 优化 sysctl
    do_optimize "${BUILD_WORK_DIR}/initrd"
    date > "${BUILD_WORK_DIR}/initrd/zpod_build_info"

    # Create the main installer initrd
    pushd "${BUILD_WORK_DIR}/initrd" > /dev/null
    time find . | cpio -o -H newc | xz -9 --check=crc32 > "${BUILD_WORK_DIR}/zpod-initrd"
    popd > /dev/null

    # --- Build zpod.tar.gz ---
    pushd "${BUILD_WORK_DIR}" > /dev/null
    ...
    if [ "$BUILD_TARGET" = "iso" ] ||  [ "$BUILD_TARGET" = "all" ]; then
        ...
        cp "${BUILD_WORK_DIR}/zpod-vmlinuz" "${BUILD_WORK_DIR}/iso/boot/"
        cp "${BUILD_WORK_DIR}/zpod-initrd" "${BUILD_WORK_DIR}/iso/boot/"
        cp "${BUILD_WORK_DIR}/disk.img.xz" "${BUILD_WORK_DIR}/iso/boot/"

        # Create GRUB config for the ISO
        cat > "${BUILD_WORK_DIR}/iso/boot/grub/grub.cfg" << EOF
set timeout=3
set default=0
...
EOF
        ...
        time grub-mkrescue --compress=xz -o "${OUTPUT_DIR}/zpod${ABI}.iso" "${BUILD_WORK_DIR}/iso" -- -volid "ZPOD_INSTALL"
    fi
